/*
 * ContribuCraft Donation plugin
 * Author:	   Lonnie Gibson
 * Author URI: http://contribucraft.com
 * Original Author:	Jerry Wood
 * License:	   GPLv2
 *
...
import com.contribucraft.contribucraft.pojo.Order;
import com.contribucraft.contribucraft.pojo.WMCPojo;
import com.contribucraft.contribucraft.pojo.WMCProcessedOrders;
import okhttp3.*;
...
public final class ContribuCraft extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        // Load the commands.
        PluginCommand command = getCommand("contribucraft");
        if (command != null) {
            command.setExecutor(new ContribuCraftCommand());
            command.setTabCompleter(new ContribuCraftCommand());
        } else {
            getLogger().severe("Unable to register command: contribucraft");
        }
        ...
        long updateInterval = (long) config.getInt("update_interval") * 20L;
        scheduler.runTaskTimerAsynchronously(instance, updateInterval, updateInterval);
